...
const TeacherSidebar = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    const { user, logout } = useAuth();
    const navigate = useNavigate();

    const navigation = [
        { name: 'الرئيسية', href: '/teacher', icon: Home },
        { name: 'إدارة الفصول', href: '/teacher/classes', icon: Video },
        { name: 'الواجبات', href: '/teacher/homework', icon: FileText },
        { name: 'الاختبارات', href: '/teacher/exams', icon: BarChart2 },
    ];
                ...
                <div className="flex-1 flex flex-col min-h-0 border-l border-gray-200 bg-white">
                    <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div className="flex items-center flex-shrink-0 px-4">
                            <img src={Logo} alt="Abnaouna Logo" className="h-20 w-auto" />
                        </div>
                    ...
                    <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
                        <div className="flex items-center">
                            <div className="ml-3">
                                ...
                                <p className="text-xs font-medium text-gray-500">معلم</p>
                            </div>
                        </div>
